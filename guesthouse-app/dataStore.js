...
const DataStore = {
    ...
    async updateRoomCleanliness(id, cleanliness) {
        ...
        if (this.cache.rooms) {
            ...
            if (idx !== -1) {
                this.cache.rooms[idx] = { ...this.cache.rooms[idx], cleanliness };
            }
        }
    ...
    // --- Housekeeping (column name varies: housekeeper_id in production, staff_id in some migrations) ---
    _housekeepingProfileId(row) {
        return row?.housekeeper_id ?? row?.staff_id ?? null;
    },
    ...
    async addHousekeepingTask(task) {
        const profileId = task.housekeeper_id ?? task.staff_id;
        const payload = {
            housekeeper_id: profileId,
    ...
    async createHousekeeperLoginToken(staffId) {
        ...
        let { error } = await supabaseClient
            ...
            .insert([{ token, housekeeper_id: staffId }]);
        if (error) {
    ...
    async getHousekeeperByToken(token) {
        ...
        const profileId = this._housekeepingProfileId(data);
    ...
    subscribeToChanges(table, callback, channelScope = 'main') {
        return supabaseClient
            .channel(`realtime:${channelScope}:${table}`)
