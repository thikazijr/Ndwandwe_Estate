...
CREATE POLICY housekeeping_task_select ON housekeeping_tasks
    FOR SELECT USING (auth.role() = 'housekeeper' AND staff_id = auth.uid());
CREATE POLICY housekeeping_task_insert ON housekeeping_tasks
    FOR INSERT WITH CHECK (auth.role() = 'housekeeper' AND staff_id = auth.uid());
CREATE POLICY housekeeping_task_update ON housekeeping_tasks
    FOR UPDATE USING (auth.role() = 'housekeeper' AND staff_id = auth.uid())
    WITH CHECK (auth.role() = 'housekeeper' AND staff_id = auth.uid());
